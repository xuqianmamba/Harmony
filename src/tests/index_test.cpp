...
TEST(IndexTest, L2NodeTest) {
    using namespace tribase;
    using namespace std;
    [[maybe_unused]] size_t nb = 20000, nq = 100, d = 16, k = 10, blockCount = 20, nodeCount = 4;
...
TEST(IndexTest, DatasetTest) {
    using namespace tribase;
    using namespace std;
    auto [codes, nb, d] = loadFvecs("/home/lichengqi/vdb/VectorDB/benchmarks/msong/origin/msong_base.fvecs");
    auto [queries, nq, _] = loadFvecs("/home/lichengqi/vdb/VectorDB/benchmarks/msong/origin/msong_query.fvecs");
    size_t k = 100;
    ...
    Index index(d, nlist, nlist, METRIC_IP, OPT_NONE, 15, 1, 1, false);
    cout << "START OF TRAIN" << endl;
    index.train(nb, codes.get());
